...
const EditarModal = ({ isOpen, onClose, petToEdit }) => {
  ...
  useEffect(() => {
    if (isOpen && petToEdit) {
      ...
      setDataNascimento(new Date(petToEdit.dateOfBirth));
      setTipo(petToEdit.type);
      setRaca(petToEdit.breed);
      setNomeDono(petToEdit.ownerName);
      setContatoDono(petToEdit.ownerContact);
    }
  }, [isOpen, petToEdit]);
  ...
  const handleAtualizar = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Erro ao atualizar o pet");
      }
    } catch (error) {
    ...
    <div style={{ display: isOpen ? "block" : "none" }}>
      <div style={{ border: "solid, 1px", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, display: "flex", justifyContent: "center", alignItems: "center", color: "white" }}>
        <div style={{ background: "linear-gradient(316.01deg, #000814 15.31%, #001E4D 86.58%)", padding: 20, borderRadius: 8, maxWidth: 600,  border: "3px solid", borderColor: "rgba(0, 202, 252, 1)" }}>
          <h1 style={{ color: "white", marginBottom: "2em" }}>Editar Pet</h1>
          ...
          <div style={{ textAlign: "right" }}>
            <button onClick={handleAtualizar} style={{ marginRight: 10, padding: "5px 10px", backgroundColor: "#007bff", border: "none", borderRadius: "4em", cursor: "pointer" }}>Atualizar</button>
