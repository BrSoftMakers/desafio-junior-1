...
const UPDATE_MUT = gql`   
    mutation CreateAnimal($id: Int!,$nome: String!, $idade: Int!, $tipo: TipoAnimal!, $raca: String!, $nomeDono: String!, $telefoneDono: String!){
        createAnimal(id: $id,nome: $nome, idade: $idade, tipo: $tipo, raca: $raca, nomeDono: $nomeDono, telefoneDono: $telefoneDono) {
            id
            nome
            idade
	        tipo
...
export default function UpdatePet() {
  ...
  const [id, setId] = useState(parseInt(value));
  ...
  const [, updade] = useMutation(UPDATE_MUT);

  const handleChangeCadastrar = async () => {
    const response = await updade({id: id, nome: nomePet, idade: parseInt(idade), tipo: tipo, raca: raca, nomeDono: nomeDono, telefoneDono: telefoneDono})
      ...
      <div className={classes.divCenter}>
        <Paper className={classes.paper} elevation={3} >
          ...
          <div className={classes.divButton}>
            <Button className={classes.button} variant="contained" color="secondary" onClick={formikPet.handleSubmit}>Atualizar</Button>
          </div>
        </Paper>
      </div>
    </div>
